...
describe("CompanySelect — rendering", () => {
  ...
  it("renders known companies as options", () => {
    render(<CompanySelect id="co" label="Company" value="" onChange={jest.fn()} companies={["Acme Ltd", "Globex"]} />);
...
describe("CompanySelect — selecting a known company", () => {
  it("calls onChange with the selected company name", async () => {
    ...
    const user = setup();
    render(<CompanySelect id="co" label="Company" value="" onChange={onChange} companies={["Acme Ltd"]} />);
...
describe("CompanySelect — pre-filled with unknown value", () => {
  it("shows the custom input when value is not in the companies list", () => {
    render(<CompanySelect id="co" label="Company" value="Custom Corp" onChange={jest.fn()} companies={["Acme Ltd"]} />);
    expect(screen.getByLabelText("Custom company name")).toBeInTheDocument();
...
describe("CompanySelect — deduplication and sorting", () => {
  it("sorts options alphabetically", () => {
    render(<CompanySelect id="co" label="Company" value="" onChange={jest.fn()} companies={["Zebra Inc", "Alpha Co", "Acme Ltd"]} />);
    const options = screen.getAllByRole("option").map((o) => o.textContent).filter((t) => !t?.includes("\u2014") && !t?.includes("Other"));
    expect(options).toEqual(["Acme Ltd", "Alpha Co", "Zebra Inc"]);
  });

  it("deduplicates companies", () => {
    render(<CompanySelect id="co" label="Company" value="" onChange={jest.fn()} companies={["Acme Ltd", "Acme Ltd", "Globex"]} />);
