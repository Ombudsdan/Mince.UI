...
describe("ViewToggle — rendering", () => {
  ...
  it("renders with the data-testid attribute", () => {
    const { getByTestId } = render(<ViewToggle options={defaultOptions} value="list" onChange={jest.fn()} />);
  ...
  it("applies an extra className to the outer container when provided", () => {
    const { container } = render(<ViewToggle options={defaultOptions} value="list" onChange={jest.fn()} className="lg:hidden" />);
...
describe("ViewToggle — active state", () => {
  it("marks the active option with aria-pressed=true", () => {
    render(<ViewToggle options={defaultOptions} value="list" onChange={jest.fn()} />);
    expect(screen.getByRole("button", { name: "Upcoming Leave" })).toHaveAttribute("aria-pressed", "true");
  });
