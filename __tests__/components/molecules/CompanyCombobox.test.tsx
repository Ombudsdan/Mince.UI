...
describe("CompanyCombobox — rendering", () => {
  ...
  it("does NOT render a datalist when suggestions is empty", () => {
    const { container } = render(<CompanyCombobox id="co" label="Company" value="" onChange={jest.fn()} suggestions={[]} />);
    expect(container.querySelector("datalist")).toBeNull();
  });

  it("renders a datalist when suggestions are provided", () => {
    const { container } = render(<CompanyCombobox id="co" label="Company" value="" onChange={jest.fn()} suggestions={["Acme Ltd", "Globex"]} />);
  ...
  it("deduplicates suggestions", () => {
    const { container } = render(<CompanyCombobox id="co" label="Company" value="" onChange={jest.fn()} suggestions={["Acme Ltd", "Acme Ltd", "Globex"]} />);
    const options = container.querySelectorAll("option");
    expect(options).toHaveLength(2);
  });
