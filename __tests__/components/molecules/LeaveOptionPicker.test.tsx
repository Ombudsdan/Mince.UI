...
describe("LeaveOptionPicker — rendering", () => {
  it("renders the label", () => {
    renderInProvider(<LeaveOptionPicker id="type" label="Type" options={typeOptions} value="" onChange={jest.fn()} />);
  ...
  it("marks the selected option as aria-pressed=true", () => {
    renderInProvider(<LeaveOptionPicker id="type" label="Type" options={typeOptions} value={LeaveType.Sick} onChange={jest.fn()} />);
    expect(screen.getByRole("button", { name: "Sick" })).toHaveAttribute("aria-pressed", "true");
    expect(screen.getByRole("button", { name: "Holiday" })).toHaveAttribute("aria-pressed", "false");
  });
});
...
describe("LeaveOptionPicker — interaction", () => {
  it("calls onChange with the clicked option value", async () => {
    ...
    renderInProvider(<LeaveOptionPicker id="type" label="Type" options={typeOptions} value="" onChange={onChange} />);
...
describe("LeaveOptionPicker — validation", () => {
  it("shows default required error via triggerAllValidations when no value selected", async () => {
    renderInProvider(<><LeaveOptionPicker id="type" label="Type" options={typeOptions} value="" onChange={jest.fn()} required /><TriggerButton /></>);
    await userEvent.click(screen.getByRole("button", { name: "Validate" }));
    expect(screen.getByText("Type is required")).toBeInTheDocument();
  });

  it("shows a custom required error message when required is a string", async () => {
    renderInProvider(<><LeaveOptionPicker id="type" label="Type" options={typeOptions} value="" onChange={jest.fn()} required="Please pick a leave type" /><TriggerButton /></>);
    await userEvent.click(screen.getByRole("button", { name: "Validate" }));
