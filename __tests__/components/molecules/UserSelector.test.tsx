...
describe("UserSelector", () => {
  it("renders the 'My Calendar' tab", () => {
    render(<UserSelector currentUser={alice} allUsers={[alice, bob]} viewingUserId={null} onSelectUser={jest.fn()} />);
  ...
  it("marks 'My Calendar' as selected when viewingUserId is null", () => {
    render(<UserSelector currentUser={alice} allUsers={[alice, bob]} viewingUserId={null} onSelectUser={jest.fn()} />);
    expect(screen.getByRole("tab", { name: "My Calendar" })).toHaveAttribute("aria-selected", "true");
  });

  it("marks the selected user's tab as selected when viewingUserId matches", () => {
    render(<UserSelector currentUser={alice} allUsers={[alice, bob]} viewingUserId="u2" onSelectUser={jest.fn()} />);
    expect(screen.getByRole("tab", { name: "Bob Jones" })).toHaveAttribute("aria-selected", "true");
    expect(screen.getByRole("tab", { name: "My Calendar" })).toHaveAttribute("aria-selected", "false");
  });

  it("calls onSelectUser(null) when 'My Calendar' tab is clicked", async () => {
    const onSelectUser = jest.fn();
    render(<UserSelector currentUser={alice} allUsers={[alice, bob]} viewingUserId="u2" onSelectUser={onSelectUser} />);
  ...
  it("calls onSelectUser with the user's id when a pinned user tab is clicked", async () => {
    const onSelectUser = jest.fn();
    render(<UserSelector currentUser={alice} allUsers={[alice, bob]} viewingUserId={null} onSelectUser={onSelectUser} />);
  ...
  it("renders nothing when the current user has no pinned users", () => {
    const aliceNoPins = { ...alice, profile: { ...alice.profile, pinnedUserIds: [] } };
    const { container } = render(<UserSelector currentUser={aliceNoPins} allUsers={[aliceNoPins, bob]} viewingUserId={null} onSelectUser={jest.fn()} />);
    expect(container.firstChild).toBeNull();
  });

  it("renders nothing when pinnedUserIds is undefined", () => {
    const aliceUndefined = { ...alice, profile: { ...alice.profile, pinnedUserIds: undefined } };
    const { container } = render(<UserSelector currentUser={aliceUndefined} allUsers={[aliceUndefined, bob]} viewingUserId={null} onSelectUser={jest.fn()} />);
    expect(container.firstChild).toBeNull();
  });
});
